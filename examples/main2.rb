...
File.open(file_output, 'w') do |f2|
    while mixing < 2
      ...
      partitions = getSets(set_mixed, proportion_training, proportion_test) # Now we've got training set and test set ready
      ...
      case way_of_execution[0].to_s
        ...
        puts "We are at combination "+ combination.to_s + " - # examples = " + trainingset.count.to_s
				accuracy[combination] = t.testing
        t.print_tree
        puts "DECISION -> "+t.get_decisions.to_s
			...
			when "V" #Validation Test
				# Then, build the tree with the examples and attributes:
        puts "We are at combination "+ combination.to_s + " - # examples = " + trainingset.count.to_s
				accuracy[combination],best_tree = several_tests("V", trainingset, testset, atts, information_classification)
